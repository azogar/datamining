  ...
  protected Instances determineOutputFormat(Instances inputFormat)
    ...
    for (i = 0; i < inputFormat.numAttributes(); i++) {
      ...
      if (m_AttributeIndices.isInRange(i)) {
        if (m_ReplaceAll) {
          atts.addElement(att.copy(att.name().replaceAll(m_Find, m_Replace)));
        } else {
          atts.addElement(att.copy(att.name().replaceFirst(m_Find, m_Replace)));
        }
      } else {
        atts.addElement((Attribute) att.copy());
      }
    }
